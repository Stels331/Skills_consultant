...
def _local_build_problem_bundle(payload: Dict[str, Any]) -> str:
    ...
    if "Technical Director quotation bottleneck" in top["title"]:
        return (
            ...
            "- no production disruption during pilot window if a pilot format is approved by the user\n"
            "- operating cost of the new flow must not exceed current hidden expert cost only if budget discipline is explicitly confirmed in the case input\n\n"
            "## assumptions_to_confirm\n"
            "- budget limit for process/tool changes is not explicitly provided in the case input\n"
            "- allowed pilot horizon is not explicitly provided in the case input\n"
            "- acceptable staffing / hiring envelope is not explicitly provided in the case input\n\n"
            "## selection_policy\n"
    ...
    if mode == "selected_solutions":
        if td_case:
            return (
                ...
                "- sol_04_client_cpq_portal (fails current budget and rollout discipline)\n"
                "- sol_01_intake_brief_and_triage (useful as a low-force step, but weaker than the selected staged path)\n\n"
                "_Generated by local-llm mode._"
            )
        return (
            "# Selected Solutions\n\n"
...
def _local_build_reporting_executive(payload: Dict[str, Any]) -> str:
    ...
    elif provisional_recommendations:
